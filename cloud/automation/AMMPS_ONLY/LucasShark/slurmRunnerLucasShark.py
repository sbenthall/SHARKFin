...
#code to start sharkfin
        ...
        try:
            ...
            start_sharkfin.wait()
            print(f"sharkfin complete successfully {start_sharkfin.pid}")
            simulation['status'] = 'Simulation Complete'
            simulation['startTime'] = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
            update_table_entity(experimentName, simulation)
            time.sleep(60)
        except Exception as e:
            print(f"failed to start sharkfin -  {e.message}")
    ...
    try:
        statspath = f'output/{experimentName}{simID}_sim_stats.txt'
        with open(statspath, 'r') as file:
        contents = file.read()
        file.close()
        simulation['simstats'] = contents
