...
to setup-economy  ;; turtles procedure for setup
  ...
  create-traders #_LiqBuyBkr [LiqBuyBkr_Setup]
...
to go
  ...
  ask traders [
    ...
    if (typeOfTrader = "LiqBuyBkr") [
      if countticks >= (LiqBuyBkr_TradeLength + tradeSpeedAdjustment) [
        set tradeStatus "Transact"

        if((ticks > 5000) and (PeriodtoEndExecution >= ticks)) [set totalCanceled (totalCanceled + tradeQuantity)]

        ;Testing Speed
        ;set speed int(random-poisson LiqBuyBkr_ArrivalRate) + 1
        set speed 0
      ]
    ]
    ...
    if((typeOfTrader = "LiquidityDemander" or typeOfTrader = "LiquiditySupplier" or typeOfTrader = "MarketMakers" or typeOfTrader = "LiqBuyBkr" ) and tradeStatus = "Buy" and tradeQuantity > 0)[
    ...
    if (tradeStatus = "Transact")[
      if countticks > (speed + tradeSpeedAdjustment)  [
        ...
        ;Prevent calls to the Broker trading function if they already hit there inventory limit
        if (typeOfTrader = "LiqBuyBkr" and sum [sharesOwned] of traders with [typeOfTrader = "LiqBuyBkr"] < LiqBuyBrk_Limit) [
          LiqBuyBkrStrategy
        ...
        if (tradePrice <= currentOrderBid and tradeStatus = "Buy") [
            set aggresiveBid (aggresiveBid + tradeQuantity)
        ]
        if (tradePrice >= currentOrderAsk and tradeStatus = "Sell") [
            set aggresiveAsk (aggresiveAsk + tradeQuantity)
        ]
      ]
...
; Distribution of Order Quantity
;------------------------------------------------------------------------------
to distOrderLiqBuyBkr
  let randDraw random 100
  set tradeQuantity 1 * LiqBuyBkr_OrderSizeMultiplier
  if(randDraw > 31)[set tradeQuantity 2 * LiqBuyBkr_OrderSizeMultiplier]
  if(randDraw > 50)[set tradeQuantity 3 * LiqBuyBkr_OrderSizeMultiplier]
  if(randDraw > 63)[set tradeQuantity 4 * LiqBuyBkr_OrderSizeMultiplier]
  if(randDraw > 73)[set tradeQuantity 5 * LiqBuyBkr_OrderSizeMultiplier]
  if(randDraw > 82)[set tradeQuantity 6 * LiqBuyBkr_OrderSizeMultiplier]
  if(randDraw > 94)[set tradeQuantity 7 * LiqBuyBkr_OrderSizeMultiplier]
end
;code by Mark Flood
...
to LiqBuyBkrStrategy
  ...
  if(timeseries = True and ticks > 5000) [
    if sum [sharesOwned] of traders with [typeOfTrader = "LiqBuyBkr"] < timeseriesvalue [
       if (resethillclimbto50 = -1)[
         set LiqBuyBkr_ProbBuy 50
       ]
       set resethillclimbto50 1
       set LiqBuyBkr_ProbBuy (LiqBuyBkr_ProbBuy + (sqrt(100 - LiqBuyBkr_ProbBuy)) / 100)
    ]
    if sum [sharesOwned] of traders with [typeOfTrader = "LiqBuyBkr"] > timeseriesvalue [
       if (resethillclimbto50 = 1)[
         set LiqBuyBkr_ProbBuy 50
       ...
       set resethillclimbto50 -1
       set LiqBuyBkr_ProbBuy (LiqBuyBkr_ProbBuy - (sqrt(abs(LiqBuyBkr_ProbBuy))) / 100)
    ]
  ]


  let randDraw random 10000
  ifelse randDraw < ((LiqBuyBkr_ProbBuy * 100) + 125) [
     set tradeStatus "Buy"
     distPriceLiqBuyBkrBuy
     distOrderLiqBuyBkr
  ][

  ]
 ; Prevent Broker from crossing inventory limit
  if(tradeQuantity + sharesOwned  > LiqBuyBrk_Limit)[
   set tradeQuantity (LiqBuyBrk_Limit - sharesOwned)
  ]
...
to-report accountValueLiqBuyBkr
  ifelse((count turtles with [typeOfTrader = "LiqBuyBkr"]) > 0)
