...
parser.add_argument(
    "--attention",
    help="chance that agent pays attention to the market on a day",
    default=0.05,
)
...
parser.add_argument(
    ...
    help="daily average growth rate of the dividend",
    default=1.000628,
)
parser.add_argument(
    "--dividend_std", help="daily standard deviation fo the dividend", default=0.011988
)

# Specific to RabbitMQ AMMPS Market
parser.add_argument("-q", "--queue", help="name of rabbitmq queue", default="rpc_queue")
parser.add_argument(
    "-r", "--rhost", help="rabbitmq server location", default="localhost"
)
...
# DEPRECATED in favor of parameters.build_population
...
def run_simulation(
    ...
    sim = AttentionSimulation(pop, FinanceModel, a=a, q=q, r=r, market=market, rng=rng)
