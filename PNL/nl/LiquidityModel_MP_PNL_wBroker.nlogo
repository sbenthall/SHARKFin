...
;------------------------------------------------------------------------------
to LiqBkr_Setup
  set typeOfTrader "LiqBkr"
  set speed (random-poisson LiqBkr_ArrivalRate) + 1
  set tradeSpeedAdjustment (random 30) + 1
  set tradeStatus "Transact"
  set orderNumber 0
  set countticks 0
...
to distOrderLiqBkr
  let randDraw random 100
  set tradeQuantity 1 * LiqBkr_OrderSizeMultiplier
  if(randDraw > 31)[set tradeQuantity 2 * LiqBkr_OrderSizeMultiplier]
  if(randDraw > 50)[set tradeQuantity 3 * LiqBkr_OrderSizeMultiplier]
  if(randDraw > 63)[set tradeQuantity 4 * LiqBkr_OrderSizeMultiplier]
  if(randDraw > 73)[set tradeQuantity 5 * LiqBkr_OrderSizeMultiplier]
  if(randDraw > 82)[set tradeQuantity 6 * LiqBkr_OrderSizeMultiplier]
  if(randDraw > 94)[set tradeQuantity 7 * LiqBkr_OrderSizeMultiplier]
end
;code by Mark Flood
...
to LiqBkrStrategy
  ...
  if(timeseries = True and ticks > 5000) [
    if sum [sharesOwned] of traders with [typeOfTrader = "LiqBkr"] < timeseriesvalue [
       if (resethillclimbto50 = -1)[
         set LiqBkr_ProbBuy 50
       ]
       set resethillclimbto50 1
       set LiqBkr_ProbBuy (LiqBkr_ProbBuy + (sqrt(100 - LiqBkr_ProbBuy)) / 100)
    ]
    if sum [sharesOwned] of traders with [typeOfTrader = "LiqBkr"] > timeseriesvalue [
       if (resethillclimbto50 = 1)[
         set LiqBkr_ProbBuy 50
       ...
       set resethillclimbto50 -1
       set LiqBkr_ProbBuy (LiqBkr_ProbBuy - (sqrt(abs(LiqBkr_ProbBuy))) / 100)
    ]
  ]

  let randDraw random 10000
  ifelse randDraw < ((LiqBkr_ProbBuy * 100) + 125) [
     set tradeStatus "Buy"
     distPriceLiqBkrBuy
     distOrderLiqBkr
  ][
...
; Reporting
;------------------------------------------------------------------------------
to-report avgSharesLiqBkr
 ifelse((count turtles with [typeOfTrader = "LiqBkr"]) > 0)
 [report precision (((sum [sharesOwned] of traders with [typeOfTrader = "LiqBkr"])) / (count traders with [typeOfTrader = "LiqBkr"]))  2
 ][report 0]
end
to-report accountValueLiqBkr
  ifelse((count turtles with [typeOfTrader = "LiqBkr"]) > 0)
  [report precision ((sum [tradeAccount] of traders with [typeOfTrader = "LiqBkr"]) / (count traders with [typeOfTrader = "LiqBkr"])) 2
  ][report 0]
end
;code by Mark Flood
...
to do-plots
  ...
  if(ticks >= 5000)[
    ...
    set-current-plot-pen "LiqBkr"
    plot avgSharesLiqBkr
  ]
